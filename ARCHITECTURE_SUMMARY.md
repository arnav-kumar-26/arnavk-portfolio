...
Generated from source inspection on 2026-09-07; refreshed 2026-09-09 after the full `TASK.md` round (all 36 tasks executed and committed), the AGENTS.md/architecture.md amendments (Vercel, Geist/PT Mono, measured heading tokens), a visual audit of `audit/*.png` (reference `ref-*` vs current `new*` screenshots), and the post-round follow-up fixes (grid rework, card/tag/footer/skills/nav refinements). Refreshed 2026-09-16 with the token-lineage correction (scraper dump → Figma Style-Dictionary CSS via Antigravity in `defcfd0` → Swiss 8-col spec in `26fd21b`) and deletion of the retired `reference-token.json` / `reference-tokens.css` scraper files.
...
├── Skills.astro (interactive — Swiss 8-col clusters `md:col-start-2/4/6 + md:col-span-2` with `reveal-group` stagger)
│   └── SkillPill.astro × N (static square tags, `rounded-button` 5px just-touched corners — not capsules — `px-4 py-2 text-[23.04px]` mono uppercase, `hover:translate-y-[-2px]`)
...
**Source:** runtime truth is `src/styles/global.css :root` vars → `tailwind.config.ts` tokens. Components use utilities only — raw hex / raw `ms` / raw easing in a component is a bug.

**Token lineage (why the early passes churned):** the Webflow scraper dump (`reference-token.json` / `reference-tokens.css`, deleted 2026-09-16) was a flat list of computed styles (color counts, isolated px values) with no grid, spacing, or hierarchy — applying it directly caused button/section/container clashes. The fix was an off-repo Figma Style-Dictionary CSS (neutral ramp, spacing, radius, viewport margins) applied to `global.css` via Antigravity in `defcfd0` ("sync Swiss tokens to CSS vars"), then the Swiss 8-column spec in `26fd21b` (`48px` outer margins, `24px` gaps, `HairlineGrid`), polished in `b498bd1 → 8c8a387 → b371d20 → c1be94c`. Do not design from the deleted scraper files; `design.md`'s screenshot estimates are likewise superseded.

- **Colors (live):** `background.DEFAULT #e6e6e6`, `background.nav #e6e6e6`, `background.dark #0f0f0f`; `text.primary #1e1e1f`, `text.secondary #29292b` (Experience bullets), `text.muted #29292b` (live override of the scraper's `#7f7f80` — metadata only), `text.inverse #f2f2f2`, `text.inverse-muted #a9a9aa`; `accent.DEFAULT #1e1e1f` (button fill — black-on-cream is the only interactive color), `accent.hover #545455`, `accent.warm #C97C4B` / `warm-light #E3A26B` (photographic extensions only); `border.hairline rgba(0,0,0,0.12)` (was `#bfbfbf`), `hairline-inverse rgba(255,255,255,0.12)`.
- **Type (live):** `display: Geist`, `mono: PT Mono`, `namemark: Geist` (self-hosted via `@fontsource`; the old Archivo/Plex-Mono/Archivo-Black plan and the NType 82 / Coolvetica track are retired — never pull from unofficial mirrors). Key measured sizes: `font-size-display 140px`, `font-size-section-title 179.194px` (Skills `h2` tier), `font-size-body-swiss 23.04px` (SkillPill text), `font-size-xs/sm 18.8px` (skill category labels). (The old `architecture.md` §4.2 values — `h3 1.75rem`, `body 0.9375rem`, `section 9rem` — are stale; live config wins.)
- **Shape/shadow:** `borderRadius` `none: 0`, `pill: 9999px`, plus `button: var(--radius-sm)` = `5px` (shipped reference `.button` treatment — square boxes with just-touched corners, the shape `SkillPill` uses; not a capsule). `boxShadow.none`. Depth = `.project-card` offset-stack (solid `bg-accent` `layer-back` plate at 8px behind `layer-front`), never shadows.
- **Layout vars (`global.css`, live):** `--section-rhythm: 14rem`, `--section-rhythm-lg: 18rem`, `--gutter-width: 48px`, `--grid-columns: 8`, `--grid-gap: 24px`, `--site-padding-desktop: 48px`, `--label-col-width: 10rem`, `--content-col-max: 65ch`. Narrow-label / wide-content split must consume these, never local widths. Grid strength 10% (`color-mix`). `body` carries `overflow-x: clip` (supports the Contact full-bleed breakout; `clip` preserves no-scroll-container behavior).
- **Motion vars:** `--ease-signature: cubic-bezier(0.16,1,0.3,1)`, `--dur-fast: 300ms`, `--dur-base: 350ms`, `--dur-entrance: 800ms`, `--stagger: 100ms`. All component transitions reference these. `prefers-reduced-motion` handled once globally (durations → 0.01ms); no per-component overrides. Footer name-mark rotation must stay `rotate(180deg)` in normal flex flow (`justify-end`), never absolute positioning (clips above viewport).
...
- `TASK.md` (all 36 tasks) is complete and committed — it is now reference-only; the live loop is closed with `GOAL COMPLETE`. Its "Source-of-Truth Reconciliation" section still cites the deleted scraper files — treat as history, not guidance.
- `reference-token.json` / `reference-tokens.css` (Webflow scraper dump) deleted 2026-09-16: untracked JSON + tracked CSS, never imported by code. Remaining doc cites repaired to point at `global.css :root` vars instead.
