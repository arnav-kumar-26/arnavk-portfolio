...
2. `border-radius` is `0` everywhere except pill buttons (`rounded-pill`, i.e. `9999px`).
   ...
   Exception: button radius `5px` (`rounded-button`), added 2026-09-08, rationale: match shipped `.button` treatment now owned by `var(--radius-sm)` in `src/styles/global.css`.
